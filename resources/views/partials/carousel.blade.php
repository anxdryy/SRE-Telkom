...
<div x-data="{
    ...
    <!-- Carousel Items -->
    <div class="flex gap-8 justify-center transition-all duration-300">
        <template x-for="alumni in visibleAlumnis" :key="alumni.id">
            <div class="border-solid border-white border-8 text-black rounded-2xl shadow-lg overflow-hidden w-full max-w-xs flex-shrink-0">
                <div class="relative">
    ...
    <div class="absolute left-4 top-1/2 transform -translate-y-1/2 z-10">
        <button @click="activeIndex = (activeIndex - 1 + alumnis.length) % alumnis.length"
            class="bg-white bg-opacity-60 text-2xl text-[#104334] w-16 h-16 rounded-full flex items-center justify-center hover:bg-gray-600">
            &larr;
        </button>
    </div>
